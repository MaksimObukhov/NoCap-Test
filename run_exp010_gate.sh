...
output_root=${2:-runs/exp010-spectral-adamw-gate-${timestamp}}
...
expected_branch=exp010/spectral-adamw-gate
...
if ! git diff --quiet || ! git diff --cached --quiet; then
  echo "tracked worktree changes would invalidate exp010" >&2
  exit 1
fi

echo "experiment=exp010"
...
mkdir -p "$output_root"
"$python_bin" analyze_spectral_adamw.py --self-test
"$python_bin" analyze_spectral_adamw.py \
...
echo "exp010 artifacts: ${output_root}"
