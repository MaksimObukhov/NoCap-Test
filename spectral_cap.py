"""Selective spectral capping for AdamW attention-V updates (exp011)."""
...
SPECTRAL_STATE_KEY = "exp011_attn_v_right_subspace"
...
def preconditioned_adamw_update(parameter, optimizer, row_slice=None):
    """Return the positive preconditioned direction A used in p <- p - lr*A."""
    ...
    if group.get("amsgrad", False) or group.get("maximize", False):
        raise RuntimeError("exp011 supports the baseline AdamW configuration only")
...
def rank_two_svd(matrix, right=None, power_iterations=6, seed=0):
    """Deterministic warm-started block-power approximation of the top two modes."""
    matrix = matrix.float()
    ...
    left, singular_values, small_vh = torch.linalg.svd(
...
def cap_leading_gap(matrix, singular_values, left, right):
    """Cap sigma1 to sigma2 and restore the original Frobenius norm."""
    matrix = matrix.float()
    gap = torch.clamp(singular_values[0] - singular_values[1], min=0.0)
    removed = gap * torch.outer(left[:, 0], right[:, 0])
    uncscaled = matrix - removed
    original_norm = torch.linalg.vector_norm(matrix)
    uncscaled_norm = torch.linalg.vector_norm(uncscaled).clamp_min(1e-30)
    scale = original_norm / uncscaled_norm
    capped = uncscaled * scale
    return capped, removed, scale


def leading_energy(matrix, power_iterations=6, seed=0):
    singular_values, _left, _right = rank_two_svd(
        matrix, power_iterations=power_iterations, seed=seed
    )
    frobenius_sq = matrix.float().square().sum().item()
    return singular_values[0].item() ** 2 / max(frobenius_sq, 1e-30)


class AttentionVSpectralCap:
    """Prepare an AdamW correction, then apply it after optimizer.step()."""
    ...
    def prepare(self):
        ...
        for layer, parameter in self.targets:
            value_update = preconditioned_adamw_update(
                parameter,
                self.optimizer,
                slice(2 * self.width, 3 * self.width),
            )
            ...
            capped, _removed, scale = cap_leading_gap(
                value_update, singular_values, left, right
            )
            correction = value_update - capped
...
def aggregate_diagnostics(records):
    if not records:
        return {}
    return {
        "spectral_cap_fraction_median": statistics.median(
            record["cap_fraction"] for record in records
        ),
        "spectral_norm_rescale_median": statistics.median(
            record["norm_rescale"] for record in records
        ),
        "spectral_sigma1_median": statistics.median(
            record["sigma1"] for record in records
        ),
        "spectral_sigma2_median": statistics.median(
            record["sigma2"] for record in records
        ),
    }
...
def synthetic_self_test():
    ...
    singular_values, left, right = rank_two_svd(
        matrix, power_iterations=12, seed=5
    )
    capped, _removed, _scale = cap_leading_gap(
        matrix, singular_values, left, right
    )
    capped_singular_values = torch.linalg.svdvals(capped)
    torch.testing.assert_close(
        capped_singular_values[0], capped_singular_values[1], rtol=2e-4, atol=2e-4
    )
    torch.testing.assert_close(
        torch.linalg.vector_norm(capped),
    ...
    unchanged, _removed, _scale = cap_leading_gap(identity, values, left, right)
